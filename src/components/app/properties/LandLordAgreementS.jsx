...
const viewAndDownloadFile = (url) => {
  window.open(url, "_blank");
}

export default function LandlordAgreements({property}) {
  ...
  return (
    <div className="bg-white rounded-[12px] p-5 w-full max-w-[420px] my-4">
      ...
      <div className="flex items-center gap-4 mb-4 px-2">
        ...
        />
        <span className="font-semibold text-[20px]">{property?.landlord?.name}</span>
        <button onClick={()=>navigate(`/landlords/${property.landlord._id}`)}>
      ...
      <div className="flex gap-2 mb-4 bg-[#F2F2F2] rounded-[10px] p-2">
        ...
        <button onClick={() => setActiveTab("conditions")} className={`text-[#181818] font-medium px-4 py-2 rounded-lg ${activeTab === "conditions" ? "bg-white px-4 py-2 rounded-lg" : ""}`}>
          Conditions
        </button>
      ...
      <div className="flex flex-col gap-3  overflow-y-scroll h-[200px] ">
        {property?.landlord?.agreements&& property?.landlord?.agreements.length>0 ? property?.landlord?.agreements?.map((agreement, idx) => (
          <div
            key={idx}
          ...
          >
            ...
            <button
              onClick={() => viewAndDownloadFile(agreement.fileUrl, agreement.fileKey)}
      ...
      <div className="flex flex-col gap-3 space-y-2 overflow-y-scroll h-[200px] ">
        <p className="font-[400] text-[16px] flex-1">Video</p>
        {property?.landlord?.propertyConditionVideos&& property?.landlord?.propertyConditionVideos.length>0 ? property?.landlord?.propertyConditionVideos?.map((agreement, idx) => (
          <div
            key={idx}
            className="flex items-center bg-[#F3F8FF] rounded-[10px] px-4 py-3"
          >
        ...
        { property?.landlord?.propertyConditionImages&& property?.landlord?.propertyConditionImages.length>0 ? property?.landlord?.propertyConditionImages?.map((agreement, idx) => (
          <div
            key={idx}
            className="flex items-center bg-[#F3F8FF] rounded-[10px] px-4 py-3"
          >
      ...
      {activeTab === "rules" && (
      <div className="flex flex-col gap-3  overflow-y-scroll h-[200px] ">
         {property?.landlord?.rules&& property?.landlord?.rules.length>0 ? property?.landlord?.rules?.map((agreement, idx) => (
          <div
            key={idx}
            className="flex items-center bg-[#F3F8FF] rounded-[10px] px-4 py-3"
          >
